...
TEST_SIGNING_KEY = "example-signing-key-for-tests"


class TestKillSwitchModes:
    def test_starts_disengaged(self):
        ks = KillSwitch(signing_key=TEST_SIGNING_KEY)
...
class TestPersistence:
    def test_save_and_load(self, tmp_path):
        ...
        ks1 = KillSwitch(signing_key=TEST_SIGNING_KEY, state_path=path)
        ks1.engage(KillSwitchMode.HALT_ALL, "persist test", "admin")
        ks2 = KillSwitch(signing_key=TEST_SIGNING_KEY, state_path=path)
        assert ks2.mode == KillSwitchMode.HALT_ALL
        assert ks2.state.reason == "persist test"

    def test_tamper_detection(self, tmp_path):
        path = tmp_path / "ks.json"
        ks = KillSwitch(signing_key=TEST_SIGNING_KEY, state_path=path)
        ks.engage(KillSwitchMode.EMERGENCY, "test", "admin")
        raw = json.loads(path.read_text())
        raw["sig"] = "tampered"
        path.write_text(json.dumps(raw))
        with pytest.raises(ValueError, match="tampered"):
            KillSwitch(signing_key=TEST_SIGNING_KEY, state_path=path)

    def test_wrong_secret_fails(self, tmp_path):
        path = tmp_path / "ks.json"
        KillSwitch(signing_key="example-signing-key-a", state_path=path).engage(
            KillSwitchMode.HALT_ALL, "test", "admin"
        )
        with pytest.raises(ValueError, match="tampered"):
            KillSwitch(signing_key="example-signing-key-b", state_path=path)
